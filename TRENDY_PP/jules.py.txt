...
__version__ = "last known fcm revision: " + ''.join(filter(str.isdigit, "$Rev: 22759 $"))

# these are taken from the jules 4.3 (and adding p_rivers, sclayer)    
...
def _make_spatial_coord(coord, name, units='degrees', tolerance = TOLERANCE_LATLON, coord_system=None):
    ...
    if step < tolerance:
        ...
        coordlist = np.linspace(min(coordset), max(coordset), np.rint((max(coordset) - min(coordset)) / step) + 1)
